module myFun
    ...
    export smooth_tract_selective_PCA_complex
    export smooth_tract_selective_PCA_complex128
    export denoisingDWI
    export denoisingDWI_complex
    export denoisingDWI_complex128
...
#         s[2]=s[2]/2f0;
    ...
    function smooth_tract_selective_PCA_complex(tmparray::Array{Complex64},tmparray2Dr::Array{Complex64},magicnumber::Int64)
        tmparrayABS = abs.(tmparray);
        tmparrayREAL = real.(tmparray);
        tmparrayIMAG = imag.(tmparray);
        tmparray2DrABS = abs.(tmparray2Dr);
        tmparray2DrREAL = real.(tmparray2Dr);
        tmparray2DrIMAG = imag.(tmparray2Dr);
        tmparraydiff = abs.(tmparray2DrABS.-tmparrayABS)
        tmparraydiff = sum(tmparraydiff,1)                  
        tmpcorrI = sortperm(tmparraydiff[:])
    
        tmparray_chosen = tmparray2DrREAL[:,tmpcorrI[1:magicnumber]]
        mean_tmparray_chosen = mean(tmparray_chosen,1);
        tmparray_chosen = tmparray_chosen .- mean_tmparray_chosen;
        u,s,v = svd(tmparray_chosen);
#         s[2]=s[2]/2f0;
        ...
        tmparray_chosen2 = u*Diagonal(s)*v';
        tmparray_chosen2 = tmparray_chosen2 .+ mean_tmparray_chosen;
        seed_smoothed_pca_REAL = tmparray_chosen2[:,1];

        tmparray_chosen = tmparray2DrIMAG[:,tmpcorrI[1:magicnumber]]
        mean_tmparray_chosen = mean(tmparray_chosen,1);
        tmparray_chosen = tmparray_chosen .- mean_tmparray_chosen;
        u,s,v = svd(tmparray_chosen);
#         s[2]=s[2]/2f0;
        ...
        tmparray_chosen2 = u*Diagonal(s)*v';
        tmparray_chosen2 = tmparray_chosen2 .+ mean_tmparray_chosen;
        seed_smoothed_pca_IMAG = tmparray_chosen2[:,1];

        seed_smoothed_pca = seed_smoothed_pca_REAL + (im*seed_smoothed_pca_IMAG);
    
        return seed_smoothed_pca
    end

    function smooth_tract_selective_PCA_complex128(tmparray::Array{Complex128},tmparray2Dr::Array{Complex128},magicnumber::Int64)
        tmparrayABS = abs.(tmparray);
        tmparrayREAL = real.(tmparray);
        tmparrayIMAG = imag.(tmparray);
        tmparray2DrABS = abs.(tmparray2Dr);
...
#         datadxyz_sm = zeros(Float32, ddim,xdim,ydim,zdim); 
    ...
    function denoisingDWI_complex(data::Array{Complex64,4})
        magicnumber = 64;
        const xr = 5
        const yr = 5
        const zr = 2
...
#         datadxyz_sm = zeros(Float32, ddim,xdim,ydim,zdim); 
        @sync @parallel for cntz = 1:zdim
            for cnty = 1:ydim
                @inbounds @fastmath @simd for cntx = 1:xdim
                    ...
                    tmparray2D = datadxyz[:,minx:maxx,miny:maxy,minz:maxz];
                    tmparray2Dr = reshape(tmparray2D,ddim,Lx*Ly*Lz);
                    tmparray_chosen_recovered = smooth_tract_selective_PCA_complex(tmparray,tmparray2Dr,magicnumber);            
                    datadxyz_sm[:,cntx,cnty,cntz]=convert(Array{Float32},abs.(tmparray_chosen_recovered));
                end
            end
        end
    ...
    function denoisingDWI_complex128(data::Array{Complex128,4})
        magicnumber = 64;
        const xr = 5
        const yr = 5
        const zr = 2
        xdim,ydim,zdim,ddim=size(data);
        datadxyz = permutedims(data,[4 1 2 3]);
        datadxyz_sm = SharedArray{Float64,4}((ddim,xdim,ydim,zdim)); # this is for parallel computation
#         datadxyz_sm = zeros(Float32, ddim,xdim,ydim,zdim); 
        @sync @parallel for cntz = 1:zdim
            for cnty = 1:ydim
                @inbounds @fastmath @simd for cntx = 1:xdim
                    ...
                    tmparray2D = datadxyz[:,minx:maxx,miny:maxy,minz:maxz];
                    tmparray2Dr = reshape(tmparray2D,ddim,Lx*Ly*Lz);
                    tmparray_chosen_recovered = smooth_tract_selective_PCA_complex128(tmparray,tmparray2Dr,magicnumber);            
                    datadxyz_sm[:,cntx,cnty,cntz]=convert(Array{Float32},abs.(tmparray_chosen_recovered));
                end
            end
        end
